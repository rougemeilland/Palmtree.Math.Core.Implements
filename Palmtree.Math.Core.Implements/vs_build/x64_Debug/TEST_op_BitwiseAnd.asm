; Listing generated by Microsoft (R) Optimizing Compiler Version 19.16.27026.1 

include listing.inc

INCLUDELIB MSVCRTD
INCLUDELIB OLDNAMES

msvcjmc	SEGMENT
__7B7A869E_ctype@h DB 01H
__457DD326_basetsd@h DB 01H
__4384A2D9_corecrt_memcpy_s@h DB 01H
__4E51A221_corecrt_wstring@h DB 01H
__2140C079_string@h DB 01H
__1887E595_winnt@h DB 01H
__9FC7C64B_processthreadsapi@h DB 01H
__FA470AEC_memoryapi@h DB 01H
__F37DAFF1_winerror@h DB 01H
__7A450CCC_winbase@h DB 01H
__B4B40122_winioctl@h DB 01H
__86261D59_stralign@h DB 01H
__1C66ECB2_pmc_debug@h DB 01H
__753CF642_test_op_bitwiseand@c DB 01H
msvcjmc	ENDS
PUBLIC	TEST_PMC_BitwiseAnd_I_X
PUBLIC	TEST_PMC_BitwiseAnd_L_X
PUBLIC	TEST_PMC_BitwiseAnd_X_I
PUBLIC	TEST_PMC_BitwiseAnd_X_L
PUBLIC	TEST_PMC_BitwiseAnd_X_X
PUBLIC	__JustMyCode_Default
PUBLIC	??_C@_0DE@FOPJPKJE@PMC_FromByteArray?$IC?L?$JF?$JM?$ILA?$IDR?$IB?$FL?$IDh?$IC?$KK@ ; `string'
PUBLIC	??_C@_0BL@EMOMBDEN@PMC_BitwiseAnd_I_X?5?$CI?$CFd?4?$CFd?$CJ@ ; `string'
PUBLIC	??_C@_0DF@BICIFIFF@PMC_BitwiseAnd_I_X?$IC?L?$JF?$JM?$ILA?$IDR?$IB?$FL?$IDh?$IC@ ; `string'
PUBLIC	??_C@_0BJ@JGINKGIA@?$IDf?$IB?$FL?$ID?$FO?$IC?L?$JD?$OA?$JHe?$IC?$KK?$II?j?$JCv?$IC?$LF?$IC?H?$IC?$KC@ ; `string'
PUBLIC	??_C@_0BL@NEEPGEFN@PMC_BitwiseAnd_L_X?5?$CI?$CFd?4?$CFd?$CJ@ ; `string'
PUBLIC	??_C@_0DF@MOBAFIPM@PMC_BitwiseAnd_L_X?$IC?L?$JF?$JM?$ILA?$IDR?$IB?$FL?$IDh?$IC@ ; `string'
PUBLIC	??_C@_0BL@BDDNFIAI@PMC_BitwiseAnd_X_I?5?$CI?$CFd?4?$CFd?$CJ@ ; `string'
PUBLIC	??_C@_0DF@FECDOABD@PMC_BitwiseAnd_X_I?$IC?L?$JF?$JM?$ILA?$IDR?$IB?$FL?$IDh?$IC@ ; `string'
PUBLIC	??_C@_0BL@PFBEJDEM@PMC_BitwiseAnd_X_L?5?$CI?$CFd?4?$CFd?$CJ@ ; `string'
PUBLIC	??_C@_0DF@PJADGMBB@PMC_BitwiseAnd_X_L?$IC?L?$JF?$JM?$ILA?$IDR?$IB?$FL?$IDh?$IC@ ; `string'
PUBLIC	??_C@_0BL@NLFBLCNO@PMC_BitwiseAnd_X_X?5?$CI?$CFd?4?$CFd?$CJ@ ; `string'
PUBLIC	??_C@_0DF@CABCFGNK@PMC_BitwiseAnd_X_X?$IC?L?$JF?$JM?$ILA?$IDR?$IB?$FL?$IDh?$IC@ ; `string'
PUBLIC	??_C@_0DC@JBMEDBHF@PMC_ToByteArray?$IC?L?$JF?$JM?$ILA?$IDR?$IB?$FL?$IDh?$IC?$KK?$IK?z@ ; `string'
EXTRN	TEST_Assert:PROC
EXTRN	FormatTestLabel:PROC
EXTRN	FormatTestMesssage:PROC
EXTRN	_RTC_CheckStackVars:PROC
EXTRN	_RTC_InitBase:PROC
EXTRN	_RTC_Shutdown:PROC
EXTRN	__CheckForDebuggerJustMyCode:PROC
EXTRN	__GSHandlerCheck:PROC
EXTRN	__security_check_cookie:PROC
EXTRN	__security_cookie:QWORD
;	COMDAT pdata
pdata	SEGMENT
$pdata$TEST_PMC_BitwiseAnd_I_X DD imagerel $LN10
	DD	imagerel $LN10+474
	DD	imagerel $unwind$TEST_PMC_BitwiseAnd_I_X
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$TEST_PMC_BitwiseAnd_L_X DD imagerel $LN10
	DD	imagerel $LN10+477
	DD	imagerel $unwind$TEST_PMC_BitwiseAnd_L_X
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$TEST_PMC_BitwiseAnd_X_I DD imagerel $LN10
	DD	imagerel $LN10+474
	DD	imagerel $unwind$TEST_PMC_BitwiseAnd_X_I
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$TEST_PMC_BitwiseAnd_X_L DD imagerel $LN10
	DD	imagerel $LN10+477
	DD	imagerel $unwind$TEST_PMC_BitwiseAnd_X_L
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$TEST_PMC_BitwiseAnd_X_X DD imagerel $LN16
	DD	imagerel $LN16+889
	DD	imagerel $unwind$TEST_PMC_BitwiseAnd_X_X
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$_EQUALS_MEMORY DD imagerel _EQUALS_MEMORY
	DD	imagerel _EQUALS_MEMORY+198
	DD	imagerel $unwind$_EQUALS_MEMORY
pdata	ENDS
;	COMDAT rtc$TMZ
rtc$TMZ	SEGMENT
_RTC_Shutdown.rtc$TMZ DQ FLAT:_RTC_Shutdown
rtc$TMZ	ENDS
;	COMDAT rtc$IMZ
rtc$IMZ	SEGMENT
_RTC_InitBase.rtc$IMZ DQ FLAT:_RTC_InitBase
rtc$IMZ	ENDS
;	COMDAT ??_C@_0DC@JBMEDBHF@PMC_ToByteArray?$IC?L?$JF?$JM?$ILA?$IDR?$IB?$FL?$IDh?$IC?$KK?$IK?z@
CONST	SEGMENT
??_C@_0DC@JBMEDBHF@PMC_ToByteArray?$IC?L?$JF?$JM?$ILA?$IDR?$IB?$FL?$IDh?$IC?$KK?$IK?z@ DB 'P'
	DB	'MC_ToByteArray', 082H, 0ccH, 095H, 09cH, 08bH, 'A', 083H, 'R', 081H
	DB	'[', 083H, 'h', 082H, 0aaH, 08aH, 0faH, 091H, 0d2H, 092H, 0caH
	DB	082H, 0e8H, 082H, 0c5H, 082H, 0cdH, 082H, 0c8H, 082H, 0a2H, '('
	DB	'%d)', 00H					; `string'
CONST	ENDS
;	COMDAT ??_C@_0DF@CABCFGNK@PMC_BitwiseAnd_X_X?$IC?L?$JF?$JM?$ILA?$IDR?$IB?$FL?$IDh?$IC@
CONST	SEGMENT
??_C@_0DF@CABCFGNK@PMC_BitwiseAnd_X_X?$IC?L?$JF?$JM?$ILA?$IDR?$IB?$FL?$IDh?$IC@ DB 'P'
	DB	'MC_BitwiseAnd_X_X', 082H, 0ccH, 095H, 09cH, 08bH, 'A', 083H, 'R'
	DB	081H, '[', 083H, 'h', 082H, 0aaH, 08aH, 0faH, 091H, 0d2H, 092H
	DB	0caH, 082H, 0e8H, 082H, 0c5H, 082H, 0cdH, 082H, 0c8H, 082H, 0a2H
	DB	'(%d)', 00H					; `string'
CONST	ENDS
;	COMDAT ??_C@_0BL@NLFBLCNO@PMC_BitwiseAnd_X_X?5?$CI?$CFd?4?$CFd?$CJ@
CONST	SEGMENT
??_C@_0BL@NLFBLCNO@PMC_BitwiseAnd_X_X?5?$CI?$CFd?4?$CFd?$CJ@ DB 'PMC_Bitw'
	DB	'iseAnd_X_X (%d.%d)', 00H			; `string'
CONST	ENDS
;	COMDAT ??_C@_0DF@PJADGMBB@PMC_BitwiseAnd_X_L?$IC?L?$JF?$JM?$ILA?$IDR?$IB?$FL?$IDh?$IC@
CONST	SEGMENT
??_C@_0DF@PJADGMBB@PMC_BitwiseAnd_X_L?$IC?L?$JF?$JM?$ILA?$IDR?$IB?$FL?$IDh?$IC@ DB 'P'
	DB	'MC_BitwiseAnd_X_L', 082H, 0ccH, 095H, 09cH, 08bH, 'A', 083H, 'R'
	DB	081H, '[', 083H, 'h', 082H, 0aaH, 08aH, 0faH, 091H, 0d2H, 092H
	DB	0caH, 082H, 0e8H, 082H, 0c5H, 082H, 0cdH, 082H, 0c8H, 082H, 0a2H
	DB	'(%d)', 00H					; `string'
CONST	ENDS
;	COMDAT ??_C@_0BL@PFBEJDEM@PMC_BitwiseAnd_X_L?5?$CI?$CFd?4?$CFd?$CJ@
CONST	SEGMENT
??_C@_0BL@PFBEJDEM@PMC_BitwiseAnd_X_L?5?$CI?$CFd?4?$CFd?$CJ@ DB 'PMC_Bitw'
	DB	'iseAnd_X_L (%d.%d)', 00H			; `string'
CONST	ENDS
;	COMDAT ??_C@_0DF@FECDOABD@PMC_BitwiseAnd_X_I?$IC?L?$JF?$JM?$ILA?$IDR?$IB?$FL?$IDh?$IC@
CONST	SEGMENT
??_C@_0DF@FECDOABD@PMC_BitwiseAnd_X_I?$IC?L?$JF?$JM?$ILA?$IDR?$IB?$FL?$IDh?$IC@ DB 'P'
	DB	'MC_BitwiseAnd_X_I', 082H, 0ccH, 095H, 09cH, 08bH, 'A', 083H, 'R'
	DB	081H, '[', 083H, 'h', 082H, 0aaH, 08aH, 0faH, 091H, 0d2H, 092H
	DB	0caH, 082H, 0e8H, 082H, 0c5H, 082H, 0cdH, 082H, 0c8H, 082H, 0a2H
	DB	'(%d)', 00H					; `string'
CONST	ENDS
;	COMDAT ??_C@_0BL@BDDNFIAI@PMC_BitwiseAnd_X_I?5?$CI?$CFd?4?$CFd?$CJ@
CONST	SEGMENT
??_C@_0BL@BDDNFIAI@PMC_BitwiseAnd_X_I?5?$CI?$CFd?4?$CFd?$CJ@ DB 'PMC_Bitw'
	DB	'iseAnd_X_I (%d.%d)', 00H			; `string'
CONST	ENDS
;	COMDAT ??_C@_0DF@MOBAFIPM@PMC_BitwiseAnd_L_X?$IC?L?$JF?$JM?$ILA?$IDR?$IB?$FL?$IDh?$IC@
CONST	SEGMENT
??_C@_0DF@MOBAFIPM@PMC_BitwiseAnd_L_X?$IC?L?$JF?$JM?$ILA?$IDR?$IB?$FL?$IDh?$IC@ DB 'P'
	DB	'MC_BitwiseAnd_L_X', 082H, 0ccH, 095H, 09cH, 08bH, 'A', 083H, 'R'
	DB	081H, '[', 083H, 'h', 082H, 0aaH, 08aH, 0faH, 091H, 0d2H, 092H
	DB	0caH, 082H, 0e8H, 082H, 0c5H, 082H, 0cdH, 082H, 0c8H, 082H, 0a2H
	DB	'(%d)', 00H					; `string'
CONST	ENDS
;	COMDAT ??_C@_0BL@NEEPGEFN@PMC_BitwiseAnd_L_X?5?$CI?$CFd?4?$CFd?$CJ@
CONST	SEGMENT
??_C@_0BL@NEEPGEFN@PMC_BitwiseAnd_L_X?5?$CI?$CFd?4?$CFd?$CJ@ DB 'PMC_Bitw'
	DB	'iseAnd_L_X (%d.%d)', 00H			; `string'
CONST	ENDS
;	COMDAT ??_C@_0BJ@JGINKGIA@?$IDf?$IB?$FL?$ID?$FO?$IC?L?$JD?$OA?$JHe?$IC?$KK?$II?j?$JCv?$IC?$LF?$IC?H?$IC?$KC@
CONST	SEGMENT
??_C@_0BJ@JGINKGIA@?$IDf?$IB?$FL?$ID?$FO?$IC?L?$JD?$OA?$JHe?$IC?$KK?$II?j?$JCv?$IC?$LF?$IC?H?$IC?$KC@ DB 083H
	DB	'f', 081H, '[', 083H, '^', 082H, 0ccH, 093H, 0e0H, 097H, 'e', 082H
	DB	0aaH, 088H, 0eaH, 092H, 'v', 082H, 0b5H, 082H, 0c8H, 082H, 0a2H
	DB	00H						; `string'
CONST	ENDS
;	COMDAT ??_C@_0DF@BICIFIFF@PMC_BitwiseAnd_I_X?$IC?L?$JF?$JM?$ILA?$IDR?$IB?$FL?$IDh?$IC@
CONST	SEGMENT
??_C@_0DF@BICIFIFF@PMC_BitwiseAnd_I_X?$IC?L?$JF?$JM?$ILA?$IDR?$IB?$FL?$IDh?$IC@ DB 'P'
	DB	'MC_BitwiseAnd_I_X', 082H, 0ccH, 095H, 09cH, 08bH, 'A', 083H, 'R'
	DB	081H, '[', 083H, 'h', 082H, 0aaH, 08aH, 0faH, 091H, 0d2H, 092H
	DB	0caH, 082H, 0e8H, 082H, 0c5H, 082H, 0cdH, 082H, 0c8H, 082H, 0a2H
	DB	'(%d)', 00H					; `string'
CONST	ENDS
;	COMDAT ??_C@_0BL@EMOMBDEN@PMC_BitwiseAnd_I_X?5?$CI?$CFd?4?$CFd?$CJ@
CONST	SEGMENT
??_C@_0BL@EMOMBDEN@PMC_BitwiseAnd_I_X?5?$CI?$CFd?4?$CFd?$CJ@ DB 'PMC_Bitw'
	DB	'iseAnd_I_X (%d.%d)', 00H			; `string'
CONST	ENDS
;	COMDAT ??_C@_0DE@FOPJPKJE@PMC_FromByteArray?$IC?L?$JF?$JM?$ILA?$IDR?$IB?$FL?$IDh?$IC?$KK@
CONST	SEGMENT
??_C@_0DE@FOPJPKJE@PMC_FromByteArray?$IC?L?$JF?$JM?$ILA?$IDR?$IB?$FL?$IDh?$IC?$KK@ DB 'P'
	DB	'MC_FromByteArray', 082H, 0ccH, 095H, 09cH, 08bH, 'A', 083H, 'R'
	DB	081H, '[', 083H, 'h', 082H, 0aaH, 08aH, 0faH, 091H, 0d2H, 092H
	DB	0caH, 082H, 0e8H, 082H, 0c5H, 082H, 0cdH, 082H, 0c8H, 082H, 0a2H
	DB	'(%d)', 00H					; `string'
CONST	ENDS
;	COMDAT xdata
xdata	SEGMENT
$unwind$_EQUALS_MEMORY DD 025053901H
	DD	011d2322H
	DD	07016001dH
	DD	05015H
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$unwind$TEST_PMC_BitwiseAnd_X_X DD 025054a19H
	DD	011d2322H
	DD	07016007bH
	DD	05015H
	DD	imagerel __GSHandlerCheck
	DD	03c0H
xdata	ENDS
;	COMDAT CONST
CONST	SEGMENT
TEST_PMC_BitwiseAnd_X_X$rtcName$0 DB 075H
	DB	00H
	ORG $+2
TEST_PMC_BitwiseAnd_X_X$rtcName$1 DB 076H
	DB	00H
	ORG $+2
TEST_PMC_BitwiseAnd_X_X$rtcName$2 DB 077H
	DB	00H
	ORG $+6
TEST_PMC_BitwiseAnd_X_X$rtcName$3 DB 061H
	DB	063H
	DB	074H
	DB	075H
	DB	061H
	DB	06cH
	DB	05fH
	DB	077H
	DB	05fH
	DB	062H
	DB	075H
	DB	066H
	DB	00H
	ORG $+3
TEST_PMC_BitwiseAnd_X_X$rtcName$4 DB 061H
	DB	063H
	DB	074H
	DB	075H
	DB	061H
	DB	06cH
	DB	05fH
	DB	077H
	DB	05fH
	DB	062H
	DB	075H
	DB	066H
	DB	05fH
	DB	073H
	DB	069H
	DB	07aH
	DB	065H
	DB	00H
	ORG $+14
TEST_PMC_BitwiseAnd_X_X$rtcVarDesc DD 01a8H
	DD	08H
	DQ	FLAT:TEST_PMC_BitwiseAnd_X_X$rtcName$4
	DD	090H
	DD	0100H
	DQ	FLAT:TEST_PMC_BitwiseAnd_X_X$rtcName$3
	DD	068H
	DD	08H
	DQ	FLAT:TEST_PMC_BitwiseAnd_X_X$rtcName$2
	DD	048H
	DD	08H
	DQ	FLAT:TEST_PMC_BitwiseAnd_X_X$rtcName$1
	DD	028H
	DD	08H
	DQ	FLAT:TEST_PMC_BitwiseAnd_X_X$rtcName$0
	ORG $+240
TEST_PMC_BitwiseAnd_X_X$rtcFrameData DD 05H
	DD	00H
	DQ	FLAT:TEST_PMC_BitwiseAnd_X_X$rtcVarDesc
CONST	ENDS
;	COMDAT xdata
xdata	SEGMENT
$unwind$TEST_PMC_BitwiseAnd_X_L DD 025053901H
	DD	011d2322H
	DD	07016002dH
	DD	05015H
xdata	ENDS
;	COMDAT CONST
CONST	SEGMENT
TEST_PMC_BitwiseAnd_X_L$rtcName$0 DB 075H
	DB	00H
	ORG $+6
TEST_PMC_BitwiseAnd_X_L$rtcName$1 DB 061H
	DB	063H
	DB	074H
	DB	075H
	DB	061H
	DB	06cH
	DB	05fH
	DB	077H
	DB	00H
	ORG $+15
TEST_PMC_BitwiseAnd_X_L$rtcVarDesc DD 048H
	DD	08H
	DQ	FLAT:TEST_PMC_BitwiseAnd_X_L$rtcName$1
	DD	028H
	DD	08H
	DQ	FLAT:TEST_PMC_BitwiseAnd_X_L$rtcName$0
	ORG $+96
TEST_PMC_BitwiseAnd_X_L$rtcFrameData DD 02H
	DD	00H
	DQ	FLAT:TEST_PMC_BitwiseAnd_X_L$rtcVarDesc
CONST	ENDS
;	COMDAT xdata
xdata	SEGMENT
$unwind$TEST_PMC_BitwiseAnd_X_I DD 025053901H
	DD	011d2322H
	DD	07016002dH
	DD	05015H
xdata	ENDS
;	COMDAT CONST
CONST	SEGMENT
TEST_PMC_BitwiseAnd_X_I$rtcName$0 DB 075H
	DB	00H
	ORG $+6
TEST_PMC_BitwiseAnd_X_I$rtcName$1 DB 061H
	DB	063H
	DB	074H
	DB	075H
	DB	061H
	DB	06cH
	DB	05fH
	DB	077H
	DB	00H
	ORG $+15
TEST_PMC_BitwiseAnd_X_I$rtcVarDesc DD 044H
	DD	04H
	DQ	FLAT:TEST_PMC_BitwiseAnd_X_I$rtcName$1
	DD	028H
	DD	08H
	DQ	FLAT:TEST_PMC_BitwiseAnd_X_I$rtcName$0
	ORG $+96
TEST_PMC_BitwiseAnd_X_I$rtcFrameData DD 02H
	DD	00H
	DQ	FLAT:TEST_PMC_BitwiseAnd_X_I$rtcVarDesc
CONST	ENDS
;	COMDAT xdata
xdata	SEGMENT
$unwind$TEST_PMC_BitwiseAnd_L_X DD 025053901H
	DD	011d2322H
	DD	07016002dH
	DD	05015H
xdata	ENDS
;	COMDAT CONST
CONST	SEGMENT
TEST_PMC_BitwiseAnd_L_X$rtcName$0 DB 076H
	DB	00H
	ORG $+6
TEST_PMC_BitwiseAnd_L_X$rtcName$1 DB 061H
	DB	063H
	DB	074H
	DB	075H
	DB	061H
	DB	06cH
	DB	05fH
	DB	077H
	DB	00H
	ORG $+15
TEST_PMC_BitwiseAnd_L_X$rtcVarDesc DD 048H
	DD	08H
	DQ	FLAT:TEST_PMC_BitwiseAnd_L_X$rtcName$1
	DD	028H
	DD	08H
	DQ	FLAT:TEST_PMC_BitwiseAnd_L_X$rtcName$0
	ORG $+96
TEST_PMC_BitwiseAnd_L_X$rtcFrameData DD 02H
	DD	00H
	DQ	FLAT:TEST_PMC_BitwiseAnd_L_X$rtcVarDesc
CONST	ENDS
;	COMDAT xdata
xdata	SEGMENT
$unwind$TEST_PMC_BitwiseAnd_I_X DD 025053901H
	DD	011d2322H
	DD	07016002dH
	DD	05015H
xdata	ENDS
;	COMDAT CONST
CONST	SEGMENT
TEST_PMC_BitwiseAnd_I_X$rtcName$0 DB 076H
	DB	00H
	ORG $+6
TEST_PMC_BitwiseAnd_I_X$rtcName$1 DB 061H
	DB	063H
	DB	074H
	DB	075H
	DB	061H
	DB	06cH
	DB	05fH
	DB	077H
	DB	00H
	ORG $+15
TEST_PMC_BitwiseAnd_I_X$rtcVarDesc DD 044H
	DD	04H
	DQ	FLAT:TEST_PMC_BitwiseAnd_I_X$rtcName$1
	DD	028H
	DD	08H
	DQ	FLAT:TEST_PMC_BitwiseAnd_I_X$rtcName$0
	ORG $+96
TEST_PMC_BitwiseAnd_I_X$rtcFrameData DD 02H
	DD	00H
	DQ	FLAT:TEST_PMC_BitwiseAnd_I_X$rtcVarDesc
CONST	ENDS
; Function compile flags: /Odt
;	COMDAT __JustMyCode_Default
_TEXT	SEGMENT
__JustMyCode_Default PROC				; COMDAT
	ret	0
__JustMyCode_Default ENDP
_TEXT	ENDS
; Function compile flags: /Odtp /RTCsu /ZI
; File z:\sources\lunor\repos\rougemeilland\palmtree.math.core.implements\palmtree.math.core.implements\pmc_debug.h
;	COMDAT _EQUALS_MEMORY
_TEXT	SEGMENT
buffer1$ = 224
count1$ = 232
buffer2$ = 240
count2$ = 248
_EQUALS_MEMORY PROC					; COMDAT

; 153  : {

	mov	QWORD PTR [rsp+32], r9
	mov	QWORD PTR [rsp+24], r8
	mov	QWORD PTR [rsp+16], rdx
	mov	QWORD PTR [rsp+8], rcx
	push	rbp
	push	rdi
	sub	rsp, 232				; 000000e8H
	lea	rbp, QWORD PTR [rsp+32]
	mov	rdi, rsp
	mov	ecx, 58					; 0000003aH
	mov	eax, -858993460				; ccccccccH
	rep stosd
	mov	rcx, QWORD PTR [rsp+264]
	lea	rcx, OFFSET FLAT:__1C66ECB2_pmc_debug@h
	call	__CheckForDebuggerJustMyCode

; 154  :     if (count1 != count2)

	mov	rax, QWORD PTR count2$[rbp]
	cmp	QWORD PTR count1$[rbp], rax
	je	SHORT $LN4@EQUALS_MEM

; 155  :         return (-1);

	mov	eax, -1
	jmp	SHORT $LN1@EQUALS_MEM
$LN4@EQUALS_MEM:
$LN2@EQUALS_MEM:

; 156  :     while (count1 > 0)

	cmp	QWORD PTR count1$[rbp], 0
	jbe	SHORT $LN3@EQUALS_MEM

; 157  :     {
; 158  :         if (*buffer1 != *buffer2)

	mov	rax, QWORD PTR buffer1$[rbp]
	movzx	eax, BYTE PTR [rax]
	mov	rcx, QWORD PTR buffer2$[rbp]
	movzx	ecx, BYTE PTR [rcx]
	cmp	eax, ecx
	je	SHORT $LN5@EQUALS_MEM

; 159  :             return (-1);

	mov	eax, -1
	jmp	SHORT $LN1@EQUALS_MEM
$LN5@EQUALS_MEM:

; 160  :         ++buffer1;

	mov	rax, QWORD PTR buffer1$[rbp]
	inc	rax
	mov	QWORD PTR buffer1$[rbp], rax

; 161  :         ++buffer2;

	mov	rax, QWORD PTR buffer2$[rbp]
	inc	rax
	mov	QWORD PTR buffer2$[rbp], rax

; 162  :         --count1;

	mov	rax, QWORD PTR count1$[rbp]
	dec	rax
	mov	QWORD PTR count1$[rbp], rax

; 163  :     }

	jmp	SHORT $LN2@EQUALS_MEM
$LN3@EQUALS_MEM:

; 164  :     return (0);

	xor	eax, eax
$LN1@EQUALS_MEM:

; 165  : }

	lea	rsp, QWORD PTR [rbp+200]
	pop	rdi
	pop	rbp
	ret	0
_EQUALS_MEMORY ENDP
_TEXT	ENDS
; Function compile flags: /Odtp /RTCsu /ZI
; File z:\sources\lunor\repos\rougemeilland\palmtree.math.core.implements\palmtree.math.core.implements\test_op_bitwiseand.c
;	COMDAT TEST_PMC_BitwiseAnd_X_X
_TEXT	SEGMENT
u$ = 8
v$ = 40
w$ = 72
actual_w_buf$ = 112
actual_w_buf_size$ = 392
result$ = 420
u_result$ = 452
v_result$ = 484
w_result$ = 516
tv176 = 916
tv161 = 916
tv142 = 916
tv92 = 916
tv74 = 916
tv150 = 920
tv132 = 920
tv82 = 920
tv64 = 920
__$ArrayPad$ = 928
env$ = 976
ep$ = 984
no$ = 992
u_buf$ = 1000
u_buf_size$ = 1008
v_buf$ = 1016
v_buf_size$ = 1024
desired_w_buf$ = 1032
desired_w_buf_size$ = 1040
TEST_PMC_BitwiseAnd_X_X PROC				; COMDAT

; 92   : {

$LN16:
	mov	QWORD PTR [rsp+32], r9
	mov	DWORD PTR [rsp+24], r8d
	mov	QWORD PTR [rsp+16], rdx
	mov	QWORD PTR [rsp+8], rcx
	push	rbp
	push	rdi
	sub	rsp, 984				; 000003d8H
	lea	rbp, QWORD PTR [rsp+32]
	mov	rdi, rsp
	mov	ecx, 246				; 000000f6H
	mov	eax, -858993460				; ccccccccH
	rep stosd
	mov	rcx, QWORD PTR [rsp+1016]
	mov	rax, QWORD PTR __security_cookie
	xor	rax, rbp
	mov	QWORD PTR __$ArrayPad$[rbp], rax
	lea	rcx, OFFSET FLAT:__753CF642_test_op_bitwiseand@c
	call	__CheckForDebuggerJustMyCode

; 93   :     HANDLE u;
; 94   :     HANDLE v;
; 95   :     HANDLE w;
; 96   :     unsigned char actual_w_buf[256];
; 97   :     size_t actual_w_buf_size;
; 98   :     PMC_STATUS_CODE result;
; 99   :     PMC_STATUS_CODE u_result;
; 100  :     PMC_STATUS_CODE v_result;
; 101  :     PMC_STATUS_CODE w_result;
; 102  :     TEST_Assert(env, FormatTestLabel("PMC_BitwiseAnd_X_X (%d.%d)", no, 1), (u_result = ep->PMC_FromByteArray(u_buf, u_buf_size, &u)) == PMC_STATUS_OK, FormatTestMesssage("PMC_FromByteArrayの復帰コードが期待通りではない(%d)", u_result));

	lea	r8, QWORD PTR u$[rbp]
	mov	rdx, QWORD PTR u_buf_size$[rbp]
	mov	rcx, QWORD PTR u_buf$[rbp]
	mov	rax, QWORD PTR ep$[rbp]
	call	QWORD PTR [rax+40]
	mov	DWORD PTR u_result$[rbp], eax
	cmp	DWORD PTR u_result$[rbp], 0
	jne	SHORT $LN6@TEST_PMC_B
	mov	DWORD PTR tv74[rbp], 1
	jmp	SHORT $LN7@TEST_PMC_B
$LN6@TEST_PMC_B:
	mov	DWORD PTR tv74[rbp], 0
$LN7@TEST_PMC_B:
	mov	edx, DWORD PTR u_result$[rbp]
	lea	rcx, OFFSET FLAT:??_C@_0DE@FOPJPKJE@PMC_FromByteArray?$IC?L?$JF?$JM?$ILA?$IDR?$IB?$FL?$IDh?$IC?$KK@
	call	FormatTestMesssage
	mov	QWORD PTR tv64[rbp], rax
	mov	r8d, 1
	mov	edx, DWORD PTR no$[rbp]
	lea	rcx, OFFSET FLAT:??_C@_0BL@NLFBLCNO@PMC_BitwiseAnd_X_X?5?$CI?$CFd?4?$CFd?$CJ@
	call	FormatTestLabel
	mov	rcx, QWORD PTR tv64[rbp]
	mov	r9, rcx
	mov	r8d, DWORD PTR tv74[rbp]
	mov	rdx, rax
	mov	rcx, QWORD PTR env$[rbp]
	call	TEST_Assert

; 103  :     TEST_Assert(env, FormatTestLabel("PMC_BitwiseAnd_X_X (%d.%d)", no, 2), (v_result = ep->PMC_FromByteArray(v_buf, v_buf_size, &v)) == PMC_STATUS_OK, FormatTestMesssage("PMC_FromByteArrayの復帰コードが期待通りではない(%d)", v_result));

	lea	r8, QWORD PTR v$[rbp]
	mov	rdx, QWORD PTR v_buf_size$[rbp]
	mov	rcx, QWORD PTR v_buf$[rbp]
	mov	rax, QWORD PTR ep$[rbp]
	call	QWORD PTR [rax+40]
	mov	DWORD PTR v_result$[rbp], eax
	cmp	DWORD PTR v_result$[rbp], 0
	jne	SHORT $LN8@TEST_PMC_B
	mov	DWORD PTR tv92[rbp], 1
	jmp	SHORT $LN9@TEST_PMC_B
$LN8@TEST_PMC_B:
	mov	DWORD PTR tv92[rbp], 0
$LN9@TEST_PMC_B:
	mov	edx, DWORD PTR v_result$[rbp]
	lea	rcx, OFFSET FLAT:??_C@_0DE@FOPJPKJE@PMC_FromByteArray?$IC?L?$JF?$JM?$ILA?$IDR?$IB?$FL?$IDh?$IC?$KK@
	call	FormatTestMesssage
	mov	QWORD PTR tv82[rbp], rax
	mov	r8d, 2
	mov	edx, DWORD PTR no$[rbp]
	lea	rcx, OFFSET FLAT:??_C@_0BL@NLFBLCNO@PMC_BitwiseAnd_X_X?5?$CI?$CFd?4?$CFd?$CJ@
	call	FormatTestLabel
	mov	rcx, QWORD PTR tv82[rbp]
	mov	r9, rcx
	mov	r8d, DWORD PTR tv92[rbp]
	mov	rdx, rax
	mov	rcx, QWORD PTR env$[rbp]
	call	TEST_Assert

; 104  :     TEST_Assert(env, FormatTestLabel("PMC_BitwiseAnd_X_X (%d.%d)", no, 3), (w_result = ep->PMC_BitwiseAnd_X_X(u, v, &w)) == PMC_STATUS_OK, FormatTestMesssage("PMC_BitwiseAnd_X_Xの復帰コードが期待通りではない(%d)", w_result));

	lea	r8, QWORD PTR w$[rbp]
	mov	rdx, QWORD PTR v$[rbp]
	mov	rcx, QWORD PTR u$[rbp]
	mov	rax, QWORD PTR ep$[rbp]
	call	QWORD PTR [rax+312]
	mov	DWORD PTR w_result$[rbp], eax
	cmp	DWORD PTR w_result$[rbp], 0
	jne	SHORT $LN10@TEST_PMC_B
	mov	DWORD PTR tv142[rbp], 1
	jmp	SHORT $LN11@TEST_PMC_B
$LN10@TEST_PMC_B:
	mov	DWORD PTR tv142[rbp], 0
$LN11@TEST_PMC_B:
	mov	edx, DWORD PTR w_result$[rbp]
	lea	rcx, OFFSET FLAT:??_C@_0DF@CABCFGNK@PMC_BitwiseAnd_X_X?$IC?L?$JF?$JM?$ILA?$IDR?$IB?$FL?$IDh?$IC@
	call	FormatTestMesssage
	mov	QWORD PTR tv132[rbp], rax
	mov	r8d, 3
	mov	edx, DWORD PTR no$[rbp]
	lea	rcx, OFFSET FLAT:??_C@_0BL@NLFBLCNO@PMC_BitwiseAnd_X_X?5?$CI?$CFd?4?$CFd?$CJ@
	call	FormatTestLabel
	mov	rcx, QWORD PTR tv132[rbp]
	mov	r9, rcx
	mov	r8d, DWORD PTR tv142[rbp]
	mov	rdx, rax
	mov	rcx, QWORD PTR env$[rbp]
	call	TEST_Assert

; 105  :     TEST_Assert(env, FormatTestLabel("PMC_BitwiseAnd_X_X (%d.%d)", no, 4), (result = ep->PMC_ToByteArray(w, actual_w_buf, sizeof(actual_w_buf), &actual_w_buf_size)) == PMC_STATUS_OK, FormatTestMesssage("PMC_ToByteArrayの復帰コードが期待通りではない(%d)", result));

	lea	r9, QWORD PTR actual_w_buf_size$[rbp]
	mov	r8d, 256				; 00000100H
	lea	rdx, QWORD PTR actual_w_buf$[rbp]
	mov	rcx, QWORD PTR w$[rbp]
	mov	rax, QWORD PTR ep$[rbp]
	call	QWORD PTR [rax+48]
	mov	DWORD PTR result$[rbp], eax
	cmp	DWORD PTR result$[rbp], 0
	jne	SHORT $LN12@TEST_PMC_B
	mov	DWORD PTR tv161[rbp], 1
	jmp	SHORT $LN13@TEST_PMC_B
$LN12@TEST_PMC_B:
	mov	DWORD PTR tv161[rbp], 0
$LN13@TEST_PMC_B:
	mov	edx, DWORD PTR result$[rbp]
	lea	rcx, OFFSET FLAT:??_C@_0DC@JBMEDBHF@PMC_ToByteArray?$IC?L?$JF?$JM?$ILA?$IDR?$IB?$FL?$IDh?$IC?$KK?$IK?z@
	call	FormatTestMesssage
	mov	QWORD PTR tv150[rbp], rax
	mov	r8d, 4
	mov	edx, DWORD PTR no$[rbp]
	lea	rcx, OFFSET FLAT:??_C@_0BL@NLFBLCNO@PMC_BitwiseAnd_X_X?5?$CI?$CFd?4?$CFd?$CJ@
	call	FormatTestLabel
	mov	rcx, QWORD PTR tv150[rbp]
	mov	r9, rcx
	mov	r8d, DWORD PTR tv161[rbp]
	mov	rdx, rax
	mov	rcx, QWORD PTR env$[rbp]
	call	TEST_Assert

; 106  :     TEST_Assert(env, FormatTestLabel("PMC_BitwiseAnd_X_X (%d.%d)", no, 5), _EQUALS_MEMORY(actual_w_buf, actual_w_buf_size, desired_w_buf, desired_w_buf_size) == 0, "データの内容が一致しない");

	mov	r9, QWORD PTR desired_w_buf_size$[rbp]
	mov	r8, QWORD PTR desired_w_buf$[rbp]
	mov	rdx, QWORD PTR actual_w_buf_size$[rbp]
	lea	rcx, QWORD PTR actual_w_buf$[rbp]
	call	_EQUALS_MEMORY
	test	eax, eax
	jne	SHORT $LN14@TEST_PMC_B
	mov	DWORD PTR tv176[rbp], 1
	jmp	SHORT $LN15@TEST_PMC_B
$LN14@TEST_PMC_B:
	mov	DWORD PTR tv176[rbp], 0
$LN15@TEST_PMC_B:
	mov	r8d, 5
	mov	edx, DWORD PTR no$[rbp]
	lea	rcx, OFFSET FLAT:??_C@_0BL@NLFBLCNO@PMC_BitwiseAnd_X_X?5?$CI?$CFd?4?$CFd?$CJ@
	call	FormatTestLabel
	lea	r9, OFFSET FLAT:??_C@_0BJ@JGINKGIA@?$IDf?$IB?$FL?$ID?$FO?$IC?L?$JD?$OA?$JHe?$IC?$KK?$II?j?$JCv?$IC?$LF?$IC?H?$IC?$KC@
	mov	r8d, DWORD PTR tv176[rbp]
	mov	rdx, rax
	mov	rcx, QWORD PTR env$[rbp]
	call	TEST_Assert

; 107  :     if (w_result == PMC_STATUS_OK)

	cmp	DWORD PTR w_result$[rbp], 0
	jne	SHORT $LN2@TEST_PMC_B

; 108  :         ep->PMC_Dispose(w);

	mov	rcx, QWORD PTR w$[rbp]
	mov	rax, QWORD PTR ep$[rbp]
	call	QWORD PTR [rax+32]
$LN2@TEST_PMC_B:

; 109  :     if (v_result == PMC_STATUS_OK)

	cmp	DWORD PTR v_result$[rbp], 0
	jne	SHORT $LN3@TEST_PMC_B

; 110  :         ep->PMC_Dispose(v);

	mov	rcx, QWORD PTR v$[rbp]
	mov	rax, QWORD PTR ep$[rbp]
	call	QWORD PTR [rax+32]
$LN3@TEST_PMC_B:

; 111  :     if (u_result == PMC_STATUS_OK)

	cmp	DWORD PTR u_result$[rbp], 0
	jne	SHORT $LN4@TEST_PMC_B

; 112  :         ep->PMC_Dispose(u);

	mov	rcx, QWORD PTR u$[rbp]
	mov	rax, QWORD PTR ep$[rbp]
	call	QWORD PTR [rax+32]
$LN4@TEST_PMC_B:

; 113  : }

	lea	rcx, QWORD PTR [rbp-32]
	lea	rdx, OFFSET FLAT:TEST_PMC_BitwiseAnd_X_X$rtcFrameData
	call	_RTC_CheckStackVars
	mov	rcx, QWORD PTR __$ArrayPad$[rbp]
	xor	rcx, rbp
	call	__security_check_cookie
	lea	rsp, QWORD PTR [rbp+952]
	pop	rdi
	pop	rbp
	ret	0
TEST_PMC_BitwiseAnd_X_X ENDP
_TEXT	ENDS
; Function compile flags: /Odtp /RTCsu /ZI
; File z:\sources\lunor\repos\rougemeilland\palmtree.math.core.implements\palmtree.math.core.implements\test_op_bitwiseand.c
;	COMDAT TEST_PMC_BitwiseAnd_X_L
_TEXT	SEGMENT
u$ = 8
actual_w$ = 40
result$ = 68
u_result$ = 100
tv134 = 308
tv92 = 308
tv74 = 308
tv82 = 312
tv64 = 312
env$ = 352
ep$ = 360
no$ = 368
u_buf$ = 376
u_buf_size$ = 384
v$ = 392
desired_w$ = 400
TEST_PMC_BitwiseAnd_X_L PROC				; COMDAT

; 79   : {

$LN10:
	mov	QWORD PTR [rsp+32], r9
	mov	DWORD PTR [rsp+24], r8d
	mov	QWORD PTR [rsp+16], rdx
	mov	QWORD PTR [rsp+8], rcx
	push	rbp
	push	rdi
	sub	rsp, 360				; 00000168H
	lea	rbp, QWORD PTR [rsp+32]
	mov	rdi, rsp
	mov	ecx, 90					; 0000005aH
	mov	eax, -858993460				; ccccccccH
	rep stosd
	mov	rcx, QWORD PTR [rsp+392]
	lea	rcx, OFFSET FLAT:__753CF642_test_op_bitwiseand@c
	call	__CheckForDebuggerJustMyCode

; 80   :     HANDLE u;
; 81   :     unsigned __int64 actual_w;
; 82   :     PMC_STATUS_CODE result;
; 83   :     PMC_STATUS_CODE u_result;
; 84   :     TEST_Assert(env, FormatTestLabel("PMC_BitwiseAnd_X_L (%d.%d)", no, 1), (u_result = ep->PMC_FromByteArray(u_buf, u_buf_size, &u)) == PMC_STATUS_OK, FormatTestMesssage("PMC_FromByteArrayの復帰コードが期待通りではない(%d)", u_result));

	lea	r8, QWORD PTR u$[rbp]
	mov	rdx, QWORD PTR u_buf_size$[rbp]
	mov	rcx, QWORD PTR u_buf$[rbp]
	mov	rax, QWORD PTR ep$[rbp]
	call	QWORD PTR [rax+40]
	mov	DWORD PTR u_result$[rbp], eax
	cmp	DWORD PTR u_result$[rbp], 0
	jne	SHORT $LN4@TEST_PMC_B
	mov	DWORD PTR tv74[rbp], 1
	jmp	SHORT $LN5@TEST_PMC_B
$LN4@TEST_PMC_B:
	mov	DWORD PTR tv74[rbp], 0
$LN5@TEST_PMC_B:
	mov	edx, DWORD PTR u_result$[rbp]
	lea	rcx, OFFSET FLAT:??_C@_0DE@FOPJPKJE@PMC_FromByteArray?$IC?L?$JF?$JM?$ILA?$IDR?$IB?$FL?$IDh?$IC?$KK@
	call	FormatTestMesssage
	mov	QWORD PTR tv64[rbp], rax
	mov	r8d, 1
	mov	edx, DWORD PTR no$[rbp]
	lea	rcx, OFFSET FLAT:??_C@_0BL@PFBEJDEM@PMC_BitwiseAnd_X_L?5?$CI?$CFd?4?$CFd?$CJ@
	call	FormatTestLabel
	mov	rcx, QWORD PTR tv64[rbp]
	mov	r9, rcx
	mov	r8d, DWORD PTR tv74[rbp]
	mov	rdx, rax
	mov	rcx, QWORD PTR env$[rbp]
	call	TEST_Assert

; 85   :     TEST_Assert(env, FormatTestLabel("PMC_BitwiseAnd_X_L (%d.%d)", no, 2), (result = ep->PMC_BitwiseAnd_X_L(u, v, &actual_w)) == PMC_STATUS_OK, FormatTestMesssage("PMC_BitwiseAnd_X_Lの復帰コードが期待通りではない(%d)", result));

	lea	r8, QWORD PTR actual_w$[rbp]
	mov	rdx, QWORD PTR v$[rbp]
	mov	rcx, QWORD PTR u$[rbp]
	mov	rax, QWORD PTR ep$[rbp]
	call	QWORD PTR [rax+304]
	mov	DWORD PTR result$[rbp], eax
	cmp	DWORD PTR result$[rbp], 0
	jne	SHORT $LN6@TEST_PMC_B
	mov	DWORD PTR tv92[rbp], 1
	jmp	SHORT $LN7@TEST_PMC_B
$LN6@TEST_PMC_B:
	mov	DWORD PTR tv92[rbp], 0
$LN7@TEST_PMC_B:
	mov	edx, DWORD PTR result$[rbp]
	lea	rcx, OFFSET FLAT:??_C@_0DF@PJADGMBB@PMC_BitwiseAnd_X_L?$IC?L?$JF?$JM?$ILA?$IDR?$IB?$FL?$IDh?$IC@
	call	FormatTestMesssage
	mov	QWORD PTR tv82[rbp], rax
	mov	r8d, 2
	mov	edx, DWORD PTR no$[rbp]
	lea	rcx, OFFSET FLAT:??_C@_0BL@PFBEJDEM@PMC_BitwiseAnd_X_L?5?$CI?$CFd?4?$CFd?$CJ@
	call	FormatTestLabel
	mov	rcx, QWORD PTR tv82[rbp]
	mov	r9, rcx
	mov	r8d, DWORD PTR tv92[rbp]
	mov	rdx, rax
	mov	rcx, QWORD PTR env$[rbp]
	call	TEST_Assert

; 86   :     TEST_Assert(env, FormatTestLabel("PMC_BitwiseAnd_X_L (%d.%d)", no, 3), actual_w == desired_w, "データの内容が一致しない");

	mov	rax, QWORD PTR desired_w$[rbp]
	cmp	QWORD PTR actual_w$[rbp], rax
	jne	SHORT $LN8@TEST_PMC_B
	mov	DWORD PTR tv134[rbp], 1
	jmp	SHORT $LN9@TEST_PMC_B
$LN8@TEST_PMC_B:
	mov	DWORD PTR tv134[rbp], 0
$LN9@TEST_PMC_B:
	mov	r8d, 3
	mov	edx, DWORD PTR no$[rbp]
	lea	rcx, OFFSET FLAT:??_C@_0BL@PFBEJDEM@PMC_BitwiseAnd_X_L?5?$CI?$CFd?4?$CFd?$CJ@
	call	FormatTestLabel
	lea	r9, OFFSET FLAT:??_C@_0BJ@JGINKGIA@?$IDf?$IB?$FL?$ID?$FO?$IC?L?$JD?$OA?$JHe?$IC?$KK?$II?j?$JCv?$IC?$LF?$IC?H?$IC?$KC@
	mov	r8d, DWORD PTR tv134[rbp]
	mov	rdx, rax
	mov	rcx, QWORD PTR env$[rbp]
	call	TEST_Assert

; 87   :     if (u_result == PMC_STATUS_OK)

	cmp	DWORD PTR u_result$[rbp], 0
	jne	SHORT $LN2@TEST_PMC_B

; 88   :         ep->PMC_Dispose(u);

	mov	rcx, QWORD PTR u$[rbp]
	mov	rax, QWORD PTR ep$[rbp]
	call	QWORD PTR [rax+32]
$LN2@TEST_PMC_B:

; 89   : }

	lea	rcx, QWORD PTR [rbp-32]
	lea	rdx, OFFSET FLAT:TEST_PMC_BitwiseAnd_X_L$rtcFrameData
	call	_RTC_CheckStackVars
	lea	rsp, QWORD PTR [rbp+328]
	pop	rdi
	pop	rbp
	ret	0
TEST_PMC_BitwiseAnd_X_L ENDP
_TEXT	ENDS
; Function compile flags: /Odtp /RTCsu /ZI
; File z:\sources\lunor\repos\rougemeilland\palmtree.math.core.implements\palmtree.math.core.implements\test_op_bitwiseand.c
;	COMDAT TEST_PMC_BitwiseAnd_X_I
_TEXT	SEGMENT
u$ = 8
actual_w$ = 36
result$ = 68
u_result$ = 100
tv134 = 308
tv92 = 308
tv74 = 308
tv82 = 312
tv64 = 312
env$ = 352
ep$ = 360
no$ = 368
u_buf$ = 376
u_buf_size$ = 384
v$ = 392
desired_w$ = 400
TEST_PMC_BitwiseAnd_X_I PROC				; COMDAT

; 66   : {

$LN10:
	mov	QWORD PTR [rsp+32], r9
	mov	DWORD PTR [rsp+24], r8d
	mov	QWORD PTR [rsp+16], rdx
	mov	QWORD PTR [rsp+8], rcx
	push	rbp
	push	rdi
	sub	rsp, 360				; 00000168H
	lea	rbp, QWORD PTR [rsp+32]
	mov	rdi, rsp
	mov	ecx, 90					; 0000005aH
	mov	eax, -858993460				; ccccccccH
	rep stosd
	mov	rcx, QWORD PTR [rsp+392]
	lea	rcx, OFFSET FLAT:__753CF642_test_op_bitwiseand@c
	call	__CheckForDebuggerJustMyCode

; 67   :     HANDLE u;
; 68   :     unsigned __int32 actual_w;
; 69   :     PMC_STATUS_CODE result;
; 70   :     PMC_STATUS_CODE u_result;
; 71   :     TEST_Assert(env, FormatTestLabel("PMC_BitwiseAnd_X_I (%d.%d)", no, 1), (u_result = ep->PMC_FromByteArray(u_buf, u_buf_size, &u)) == PMC_STATUS_OK, FormatTestMesssage("PMC_FromByteArrayの復帰コードが期待通りではない(%d)", u_result));

	lea	r8, QWORD PTR u$[rbp]
	mov	rdx, QWORD PTR u_buf_size$[rbp]
	mov	rcx, QWORD PTR u_buf$[rbp]
	mov	rax, QWORD PTR ep$[rbp]
	call	QWORD PTR [rax+40]
	mov	DWORD PTR u_result$[rbp], eax
	cmp	DWORD PTR u_result$[rbp], 0
	jne	SHORT $LN4@TEST_PMC_B
	mov	DWORD PTR tv74[rbp], 1
	jmp	SHORT $LN5@TEST_PMC_B
$LN4@TEST_PMC_B:
	mov	DWORD PTR tv74[rbp], 0
$LN5@TEST_PMC_B:
	mov	edx, DWORD PTR u_result$[rbp]
	lea	rcx, OFFSET FLAT:??_C@_0DE@FOPJPKJE@PMC_FromByteArray?$IC?L?$JF?$JM?$ILA?$IDR?$IB?$FL?$IDh?$IC?$KK@
	call	FormatTestMesssage
	mov	QWORD PTR tv64[rbp], rax
	mov	r8d, 1
	mov	edx, DWORD PTR no$[rbp]
	lea	rcx, OFFSET FLAT:??_C@_0BL@BDDNFIAI@PMC_BitwiseAnd_X_I?5?$CI?$CFd?4?$CFd?$CJ@
	call	FormatTestLabel
	mov	rcx, QWORD PTR tv64[rbp]
	mov	r9, rcx
	mov	r8d, DWORD PTR tv74[rbp]
	mov	rdx, rax
	mov	rcx, QWORD PTR env$[rbp]
	call	TEST_Assert

; 72   :     TEST_Assert(env, FormatTestLabel("PMC_BitwiseAnd_X_I (%d.%d)", no, 2), (result = ep->PMC_BitwiseAnd_X_I(u, v, &actual_w)) == PMC_STATUS_OK, FormatTestMesssage("PMC_BitwiseAnd_X_Iの復帰コードが期待通りではない(%d)", result));

	lea	r8, QWORD PTR actual_w$[rbp]
	mov	edx, DWORD PTR v$[rbp]
	mov	rcx, QWORD PTR u$[rbp]
	mov	rax, QWORD PTR ep$[rbp]
	call	QWORD PTR [rax+296]
	mov	DWORD PTR result$[rbp], eax
	cmp	DWORD PTR result$[rbp], 0
	jne	SHORT $LN6@TEST_PMC_B
	mov	DWORD PTR tv92[rbp], 1
	jmp	SHORT $LN7@TEST_PMC_B
$LN6@TEST_PMC_B:
	mov	DWORD PTR tv92[rbp], 0
$LN7@TEST_PMC_B:
	mov	edx, DWORD PTR result$[rbp]
	lea	rcx, OFFSET FLAT:??_C@_0DF@FECDOABD@PMC_BitwiseAnd_X_I?$IC?L?$JF?$JM?$ILA?$IDR?$IB?$FL?$IDh?$IC@
	call	FormatTestMesssage
	mov	QWORD PTR tv82[rbp], rax
	mov	r8d, 2
	mov	edx, DWORD PTR no$[rbp]
	lea	rcx, OFFSET FLAT:??_C@_0BL@BDDNFIAI@PMC_BitwiseAnd_X_I?5?$CI?$CFd?4?$CFd?$CJ@
	call	FormatTestLabel
	mov	rcx, QWORD PTR tv82[rbp]
	mov	r9, rcx
	mov	r8d, DWORD PTR tv92[rbp]
	mov	rdx, rax
	mov	rcx, QWORD PTR env$[rbp]
	call	TEST_Assert

; 73   :     TEST_Assert(env, FormatTestLabel("PMC_BitwiseAnd_X_I (%d.%d)", no, 3), actual_w == desired_w, "データの内容が一致しない");

	mov	eax, DWORD PTR desired_w$[rbp]
	cmp	DWORD PTR actual_w$[rbp], eax
	jne	SHORT $LN8@TEST_PMC_B
	mov	DWORD PTR tv134[rbp], 1
	jmp	SHORT $LN9@TEST_PMC_B
$LN8@TEST_PMC_B:
	mov	DWORD PTR tv134[rbp], 0
$LN9@TEST_PMC_B:
	mov	r8d, 3
	mov	edx, DWORD PTR no$[rbp]
	lea	rcx, OFFSET FLAT:??_C@_0BL@BDDNFIAI@PMC_BitwiseAnd_X_I?5?$CI?$CFd?4?$CFd?$CJ@
	call	FormatTestLabel
	lea	r9, OFFSET FLAT:??_C@_0BJ@JGINKGIA@?$IDf?$IB?$FL?$ID?$FO?$IC?L?$JD?$OA?$JHe?$IC?$KK?$II?j?$JCv?$IC?$LF?$IC?H?$IC?$KC@
	mov	r8d, DWORD PTR tv134[rbp]
	mov	rdx, rax
	mov	rcx, QWORD PTR env$[rbp]
	call	TEST_Assert

; 74   :     if (u_result == PMC_STATUS_OK)

	cmp	DWORD PTR u_result$[rbp], 0
	jne	SHORT $LN2@TEST_PMC_B

; 75   :         ep->PMC_Dispose(u);

	mov	rcx, QWORD PTR u$[rbp]
	mov	rax, QWORD PTR ep$[rbp]
	call	QWORD PTR [rax+32]
$LN2@TEST_PMC_B:

; 76   : }

	lea	rcx, QWORD PTR [rbp-32]
	lea	rdx, OFFSET FLAT:TEST_PMC_BitwiseAnd_X_I$rtcFrameData
	call	_RTC_CheckStackVars
	lea	rsp, QWORD PTR [rbp+328]
	pop	rdi
	pop	rbp
	ret	0
TEST_PMC_BitwiseAnd_X_I ENDP
_TEXT	ENDS
; Function compile flags: /Odtp /RTCsu /ZI
; File z:\sources\lunor\repos\rougemeilland\palmtree.math.core.implements\palmtree.math.core.implements\test_op_bitwiseand.c
;	COMDAT TEST_PMC_BitwiseAnd_L_X
_TEXT	SEGMENT
v$ = 8
actual_w$ = 40
result$ = 68
v_result$ = 100
tv134 = 308
tv92 = 308
tv74 = 308
tv82 = 312
tv64 = 312
env$ = 352
ep$ = 360
no$ = 368
u$ = 376
v_buf$ = 384
v_buf_size$ = 392
desired_w$ = 400
TEST_PMC_BitwiseAnd_L_X PROC				; COMDAT

; 53   : {

$LN10:
	mov	QWORD PTR [rsp+32], r9
	mov	DWORD PTR [rsp+24], r8d
	mov	QWORD PTR [rsp+16], rdx
	mov	QWORD PTR [rsp+8], rcx
	push	rbp
	push	rdi
	sub	rsp, 360				; 00000168H
	lea	rbp, QWORD PTR [rsp+32]
	mov	rdi, rsp
	mov	ecx, 90					; 0000005aH
	mov	eax, -858993460				; ccccccccH
	rep stosd
	mov	rcx, QWORD PTR [rsp+392]
	lea	rcx, OFFSET FLAT:__753CF642_test_op_bitwiseand@c
	call	__CheckForDebuggerJustMyCode

; 54   :     HANDLE v;
; 55   :     unsigned __int64 actual_w;
; 56   :     PMC_STATUS_CODE result;
; 57   :     PMC_STATUS_CODE v_result;
; 58   :     TEST_Assert(env, FormatTestLabel("PMC_BitwiseAnd_L_X (%d.%d)", no, 1), (v_result = ep->PMC_FromByteArray(v_buf, v_buf_size, &v)) == PMC_STATUS_OK, FormatTestMesssage("PMC_FromByteArrayの復帰コードが期待通りではない(%d)", v_result));

	lea	r8, QWORD PTR v$[rbp]
	mov	rdx, QWORD PTR v_buf_size$[rbp]
	mov	rcx, QWORD PTR v_buf$[rbp]
	mov	rax, QWORD PTR ep$[rbp]
	call	QWORD PTR [rax+40]
	mov	DWORD PTR v_result$[rbp], eax
	cmp	DWORD PTR v_result$[rbp], 0
	jne	SHORT $LN4@TEST_PMC_B
	mov	DWORD PTR tv74[rbp], 1
	jmp	SHORT $LN5@TEST_PMC_B
$LN4@TEST_PMC_B:
	mov	DWORD PTR tv74[rbp], 0
$LN5@TEST_PMC_B:
	mov	edx, DWORD PTR v_result$[rbp]
	lea	rcx, OFFSET FLAT:??_C@_0DE@FOPJPKJE@PMC_FromByteArray?$IC?L?$JF?$JM?$ILA?$IDR?$IB?$FL?$IDh?$IC?$KK@
	call	FormatTestMesssage
	mov	QWORD PTR tv64[rbp], rax
	mov	r8d, 1
	mov	edx, DWORD PTR no$[rbp]
	lea	rcx, OFFSET FLAT:??_C@_0BL@NEEPGEFN@PMC_BitwiseAnd_L_X?5?$CI?$CFd?4?$CFd?$CJ@
	call	FormatTestLabel
	mov	rcx, QWORD PTR tv64[rbp]
	mov	r9, rcx
	mov	r8d, DWORD PTR tv74[rbp]
	mov	rdx, rax
	mov	rcx, QWORD PTR env$[rbp]
	call	TEST_Assert

; 59   :     TEST_Assert(env, FormatTestLabel("PMC_BitwiseAnd_L_X (%d.%d)", no, 2), (result = ep->PMC_BitwiseAnd_L_X(u, v, &actual_w)) == PMC_STATUS_OK, FormatTestMesssage("PMC_BitwiseAnd_L_Xの復帰コードが期待通りではない(%d)", result));

	lea	r8, QWORD PTR actual_w$[rbp]
	mov	rdx, QWORD PTR v$[rbp]
	mov	rcx, QWORD PTR u$[rbp]
	mov	rax, QWORD PTR ep$[rbp]
	call	QWORD PTR [rax+288]
	mov	DWORD PTR result$[rbp], eax
	cmp	DWORD PTR result$[rbp], 0
	jne	SHORT $LN6@TEST_PMC_B
	mov	DWORD PTR tv92[rbp], 1
	jmp	SHORT $LN7@TEST_PMC_B
$LN6@TEST_PMC_B:
	mov	DWORD PTR tv92[rbp], 0
$LN7@TEST_PMC_B:
	mov	edx, DWORD PTR result$[rbp]
	lea	rcx, OFFSET FLAT:??_C@_0DF@MOBAFIPM@PMC_BitwiseAnd_L_X?$IC?L?$JF?$JM?$ILA?$IDR?$IB?$FL?$IDh?$IC@
	call	FormatTestMesssage
	mov	QWORD PTR tv82[rbp], rax
	mov	r8d, 2
	mov	edx, DWORD PTR no$[rbp]
	lea	rcx, OFFSET FLAT:??_C@_0BL@NEEPGEFN@PMC_BitwiseAnd_L_X?5?$CI?$CFd?4?$CFd?$CJ@
	call	FormatTestLabel
	mov	rcx, QWORD PTR tv82[rbp]
	mov	r9, rcx
	mov	r8d, DWORD PTR tv92[rbp]
	mov	rdx, rax
	mov	rcx, QWORD PTR env$[rbp]
	call	TEST_Assert

; 60   :     TEST_Assert(env, FormatTestLabel("PMC_BitwiseAnd_L_X (%d.%d)", no, 3), actual_w == desired_w, "データの内容が一致しない");

	mov	rax, QWORD PTR desired_w$[rbp]
	cmp	QWORD PTR actual_w$[rbp], rax
	jne	SHORT $LN8@TEST_PMC_B
	mov	DWORD PTR tv134[rbp], 1
	jmp	SHORT $LN9@TEST_PMC_B
$LN8@TEST_PMC_B:
	mov	DWORD PTR tv134[rbp], 0
$LN9@TEST_PMC_B:
	mov	r8d, 3
	mov	edx, DWORD PTR no$[rbp]
	lea	rcx, OFFSET FLAT:??_C@_0BL@NEEPGEFN@PMC_BitwiseAnd_L_X?5?$CI?$CFd?4?$CFd?$CJ@
	call	FormatTestLabel
	lea	r9, OFFSET FLAT:??_C@_0BJ@JGINKGIA@?$IDf?$IB?$FL?$ID?$FO?$IC?L?$JD?$OA?$JHe?$IC?$KK?$II?j?$JCv?$IC?$LF?$IC?H?$IC?$KC@
	mov	r8d, DWORD PTR tv134[rbp]
	mov	rdx, rax
	mov	rcx, QWORD PTR env$[rbp]
	call	TEST_Assert

; 61   :     if (v_result == PMC_STATUS_OK)

	cmp	DWORD PTR v_result$[rbp], 0
	jne	SHORT $LN2@TEST_PMC_B

; 62   :         ep->PMC_Dispose(v);

	mov	rcx, QWORD PTR v$[rbp]
	mov	rax, QWORD PTR ep$[rbp]
	call	QWORD PTR [rax+32]
$LN2@TEST_PMC_B:

; 63   : }

	lea	rcx, QWORD PTR [rbp-32]
	lea	rdx, OFFSET FLAT:TEST_PMC_BitwiseAnd_L_X$rtcFrameData
	call	_RTC_CheckStackVars
	lea	rsp, QWORD PTR [rbp+328]
	pop	rdi
	pop	rbp
	ret	0
TEST_PMC_BitwiseAnd_L_X ENDP
_TEXT	ENDS
; Function compile flags: /Odtp /RTCsu /ZI
; File z:\sources\lunor\repos\rougemeilland\palmtree.math.core.implements\palmtree.math.core.implements\test_op_bitwiseand.c
;	COMDAT TEST_PMC_BitwiseAnd_I_X
_TEXT	SEGMENT
v$ = 8
actual_w$ = 36
result$ = 68
v_result$ = 100
tv134 = 308
tv92 = 308
tv74 = 308
tv82 = 312
tv64 = 312
env$ = 352
ep$ = 360
no$ = 368
u$ = 376
v_buf$ = 384
v_buf_size$ = 392
desired_w$ = 400
TEST_PMC_BitwiseAnd_I_X PROC				; COMDAT

; 40   : {

$LN10:
	mov	DWORD PTR [rsp+32], r9d
	mov	DWORD PTR [rsp+24], r8d
	mov	QWORD PTR [rsp+16], rdx
	mov	QWORD PTR [rsp+8], rcx
	push	rbp
	push	rdi
	sub	rsp, 360				; 00000168H
	lea	rbp, QWORD PTR [rsp+32]
	mov	rdi, rsp
	mov	ecx, 90					; 0000005aH
	mov	eax, -858993460				; ccccccccH
	rep stosd
	mov	rcx, QWORD PTR [rsp+392]
	lea	rcx, OFFSET FLAT:__753CF642_test_op_bitwiseand@c
	call	__CheckForDebuggerJustMyCode

; 41   :     HANDLE v;
; 42   :     unsigned __int32 actual_w;
; 43   :     PMC_STATUS_CODE result;
; 44   :     PMC_STATUS_CODE v_result;
; 45   :     TEST_Assert(env, FormatTestLabel("PMC_BitwiseAnd_I_X (%d.%d)", no, 1), (v_result = ep->PMC_FromByteArray(v_buf, v_buf_size, &v)) == PMC_STATUS_OK, FormatTestMesssage("PMC_FromByteArrayの復帰コードが期待通りではない(%d)", v_result));

	lea	r8, QWORD PTR v$[rbp]
	mov	rdx, QWORD PTR v_buf_size$[rbp]
	mov	rcx, QWORD PTR v_buf$[rbp]
	mov	rax, QWORD PTR ep$[rbp]
	call	QWORD PTR [rax+40]
	mov	DWORD PTR v_result$[rbp], eax
	cmp	DWORD PTR v_result$[rbp], 0
	jne	SHORT $LN4@TEST_PMC_B
	mov	DWORD PTR tv74[rbp], 1
	jmp	SHORT $LN5@TEST_PMC_B
$LN4@TEST_PMC_B:
	mov	DWORD PTR tv74[rbp], 0
$LN5@TEST_PMC_B:
	mov	edx, DWORD PTR v_result$[rbp]
	lea	rcx, OFFSET FLAT:??_C@_0DE@FOPJPKJE@PMC_FromByteArray?$IC?L?$JF?$JM?$ILA?$IDR?$IB?$FL?$IDh?$IC?$KK@
	call	FormatTestMesssage
	mov	QWORD PTR tv64[rbp], rax
	mov	r8d, 1
	mov	edx, DWORD PTR no$[rbp]
	lea	rcx, OFFSET FLAT:??_C@_0BL@EMOMBDEN@PMC_BitwiseAnd_I_X?5?$CI?$CFd?4?$CFd?$CJ@
	call	FormatTestLabel
	mov	rcx, QWORD PTR tv64[rbp]
	mov	r9, rcx
	mov	r8d, DWORD PTR tv74[rbp]
	mov	rdx, rax
	mov	rcx, QWORD PTR env$[rbp]
	call	TEST_Assert

; 46   :     TEST_Assert(env, FormatTestLabel("PMC_BitwiseAnd_I_X (%d.%d)", no, 2), (result = ep->PMC_BitwiseAnd_I_X(u, v, &actual_w)) == PMC_STATUS_OK, FormatTestMesssage("PMC_BitwiseAnd_I_Xの復帰コードが期待通りではない(%d)", result));

	lea	r8, QWORD PTR actual_w$[rbp]
	mov	rdx, QWORD PTR v$[rbp]
	mov	ecx, DWORD PTR u$[rbp]
	mov	rax, QWORD PTR ep$[rbp]
	call	QWORD PTR [rax+280]
	mov	DWORD PTR result$[rbp], eax
	cmp	DWORD PTR result$[rbp], 0
	jne	SHORT $LN6@TEST_PMC_B
	mov	DWORD PTR tv92[rbp], 1
	jmp	SHORT $LN7@TEST_PMC_B
$LN6@TEST_PMC_B:
	mov	DWORD PTR tv92[rbp], 0
$LN7@TEST_PMC_B:
	mov	edx, DWORD PTR result$[rbp]
	lea	rcx, OFFSET FLAT:??_C@_0DF@BICIFIFF@PMC_BitwiseAnd_I_X?$IC?L?$JF?$JM?$ILA?$IDR?$IB?$FL?$IDh?$IC@
	call	FormatTestMesssage
	mov	QWORD PTR tv82[rbp], rax
	mov	r8d, 2
	mov	edx, DWORD PTR no$[rbp]
	lea	rcx, OFFSET FLAT:??_C@_0BL@EMOMBDEN@PMC_BitwiseAnd_I_X?5?$CI?$CFd?4?$CFd?$CJ@
	call	FormatTestLabel
	mov	rcx, QWORD PTR tv82[rbp]
	mov	r9, rcx
	mov	r8d, DWORD PTR tv92[rbp]
	mov	rdx, rax
	mov	rcx, QWORD PTR env$[rbp]
	call	TEST_Assert

; 47   :     TEST_Assert(env, FormatTestLabel("PMC_BitwiseAnd_I_X (%d.%d)", no, 3), actual_w == desired_w, "データの内容が一致しない");

	mov	eax, DWORD PTR desired_w$[rbp]
	cmp	DWORD PTR actual_w$[rbp], eax
	jne	SHORT $LN8@TEST_PMC_B
	mov	DWORD PTR tv134[rbp], 1
	jmp	SHORT $LN9@TEST_PMC_B
$LN8@TEST_PMC_B:
	mov	DWORD PTR tv134[rbp], 0
$LN9@TEST_PMC_B:
	mov	r8d, 3
	mov	edx, DWORD PTR no$[rbp]
	lea	rcx, OFFSET FLAT:??_C@_0BL@EMOMBDEN@PMC_BitwiseAnd_I_X?5?$CI?$CFd?4?$CFd?$CJ@
	call	FormatTestLabel
	lea	r9, OFFSET FLAT:??_C@_0BJ@JGINKGIA@?$IDf?$IB?$FL?$ID?$FO?$IC?L?$JD?$OA?$JHe?$IC?$KK?$II?j?$JCv?$IC?$LF?$IC?H?$IC?$KC@
	mov	r8d, DWORD PTR tv134[rbp]
	mov	rdx, rax
	mov	rcx, QWORD PTR env$[rbp]
	call	TEST_Assert

; 48   :     if (v_result == PMC_STATUS_OK)

	cmp	DWORD PTR v_result$[rbp], 0
	jne	SHORT $LN2@TEST_PMC_B

; 49   :         ep->PMC_Dispose(v);

	mov	rcx, QWORD PTR v$[rbp]
	mov	rax, QWORD PTR ep$[rbp]
	call	QWORD PTR [rax+32]
$LN2@TEST_PMC_B:

; 50   : }

	lea	rcx, QWORD PTR [rbp-32]
	lea	rdx, OFFSET FLAT:TEST_PMC_BitwiseAnd_I_X$rtcFrameData
	call	_RTC_CheckStackVars
	lea	rsp, QWORD PTR [rbp+328]
	pop	rdi
	pop	rbp
	ret	0
TEST_PMC_BitwiseAnd_I_X ENDP
_TEXT	ENDS
END
