...
PUBLIC	??_C@_0BF@IMADJABL@PMC_ParseDN1?5?$CI?$CFd?4?$CFd?$CJ@ ; `string'
...
PUBLIC	??_C@_0BF@GHDECLBI@PMC_ParseDN2?5?$CI?$CFd?4?$CFd?$CJ@ ; `string'
...
_BSS	ENDS
;	COMDAT ??_C@_0BF@GHDECLBI@PMC_ParseDN2?5?$CI?$CFd?4?$CFd?$CJ@
CONST	SEGMENT
??_C@_0BF@GHDECLBI@PMC_ParseDN2?5?$CI?$CFd?4?$CFd?$CJ@ DB 'PMC_ParseDN2 ('
	DB	'%d.%d)', 00H				; `string'
CONST	ENDS
...
??_C@_0CN@JELPCGPD@PMC_To_X_B?$IC?L?$JF?$JM?$ILA?$IDR?$IB?$FL?$IDh?$IC?$KK?$IK?z?$JB?R?$JC?J?$IC@ DB 'P'
	...
	DB	'%d)', 00H					; `string'
CONST	ENDS
;	COMDAT ??_C@_0BF@IMADJABL@PMC_ParseDN1?5?$CI?$CFd?4?$CFd?$CJ@
CONST	SEGMENT
??_C@_0BF@IMADJABL@PMC_ParseDN1?5?$CI?$CFd?4?$CFd?$CJ@ DB 'PMC_ParseDN1 ('
	DB	'%d.%d)', 00H				; `string'
CONST	ENDS
...
_TEST_PMC_ParseDN2 PROC					; COMDAT
...
; 76   :     TEST_Assert(env, FormatTestLabel("PMC_ParseDN2 (%d.%d)", no, 1), (x_result = ep->PMC_TryParse(str, styles, &opt, &x)) == desired_result_code, FormatTestMesssage("PMC_TryParseの復帰コードが期待通りではない(%d)", x_result));
	...
	push	OFFSET ??_C@_0BF@GHDECLBI@PMC_ParseDN2?5?$CI?$CFd?4?$CFd?$CJ@
...
; 80   :         TEST_Assert(env, FormatTestLabel("PMC_ParseDN2 (%d.%d)", no, 3), _EQUALS_MEMORY(actual_buf, actual_buf_size, desired_buf, desired_buf_size) == 0, "データの内容が一致しない");
	...
	mov	edx, DWORD PTR ?actual_buf_size@?1??TEST_PMC_ParseDN2@@9@9
	add	esp, 16					; 00000010H
...
; 105  :     while (count1 > 0)
	...
	mov	esi, OFFSET ?actual_buf@?1??TEST_PMC_ParseDN2@@9@9
	sub	esi, ecx
$LL6@TEST_PMC_P:
...
; File z:\sources\lunor\repos\rougemeilland\palmtree.math.core.implements\palmtree.math.core.implements\pmc_debug.h
; File z:\sources\lunor\repos\rougemeilland\palmtree.math.core.implements\palmtree.math.core.implements\test_op_parsedn.c
;	COMDAT _TEST_PMC_ParseDN1
_TEXT	SEGMENT
_env$GSCopy$1$ = -68					; size = 4
_str$GSCopy$1$ = -64					; size = 4
_negative_sign$GSCopy$1$ = -60				; size = 4
_x$ = -56						; size = 4
_ep$GSCopy$1$ = -52					; size = 4
_x_result$1$ = -52					; size = 4
_no$GSCopy$1$ = -48					; size = 4
_opt$ = -44						; size = 40
__$ArrayPad$ = -4					; size = 4
_env$ = 8						; size = 4
...
_TEST_PMC_ParseDN1 PROC					; COMDAT
...
; 46   :     lstrcpyW(opt.GroupSeparator, group_separator);
	...
	mov	eax, DWORD PTR _no$[ebp]
	push	esi
	mov	esi, DWORD PTR _decimal_separator$[ebp]
	mov	DWORD PTR _no$GSCopy$1$[ebp], eax
	mov	eax, DWORD PTR _str$[ebp]
	push	edi
	mov	edi, DWORD PTR _positive_sign$[ebp]
	mov	DWORD PTR _str$GSCopy$1$[ebp], eax
	mov	eax, DWORD PTR _group_separator$[ebp]
	push	eax
	lea	eax, DWORD PTR _opt$[ebp+4]
	mov	DWORD PTR _negative_sign$GSCopy$1$[ebp], ecx
	push	eax
	call	ebx
...
; 51   :     lstrcpyW(opt.NegativeSign, negative_sign);
	...
	push	DWORD PTR _negative_sign$GSCopy$1$[ebp]
	lea	eax, DWORD PTR _opt$[ebp+22]
	push	eax
	call	ebx
...
; 52   :     TEST_Assert(env, FormatTestLabel("PMC_ParseDN1 (%d.%d)", no, 1), (x_result = ep->PMC_TryParse(str, styles, &opt, &x)) == desired_result_code, FormatTestMesssage("PMC_TryParseの復帰コードが期待通りではない(%d)", x_result));
	...
	mov	ebx, DWORD PTR _ep$GSCopy$1$[ebp]
	lea	eax, DWORD PTR _x$[ebp]
	push	eax
	lea	eax, DWORD PTR _opt$[ebp]
	push	eax
	push	DWORD PTR _styles$[ebp]
	mov	eax, DWORD PTR [ebx+164]
	push	DWORD PTR _str$GSCopy$1$[ebp]
	call	eax
	mov	edi, eax
	push	edi
	push	OFFSET ??_C@_0CP@JCNEMHLF@PMC_TryParse?$IC?L?$JF?$JM?$ILA?$IDR?$IB?$FL?$IDh?$IC?$KK?$IK?z?$JB?R?$JC@
	...
	push	1
	push	DWORD PTR _no$GSCopy$1$[ebp]
	push	OFFSET ??_C@_0BF@IMADJABL@PMC_ParseDN1?5?$CI?$CFd?4?$CFd?$CJ@
	call	_FormatTestLabel
	mov	edi, DWORD PTR _env$GSCopy$1$[ebp]
	add	esp, 12					; 0000000cH
	push	eax
...
; 105  :     while (count1 > 0)
	...
	test	edx, edx
	je	SHORT $LN7@TEST_PMC_P
	mov	esi, OFFSET ?actual_buf@?1??TEST_PMC_ParseDN1@@9@9
	sub	esi, eax
	npad	3
$LL6@TEST_PMC_P:
